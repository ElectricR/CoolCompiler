...
std::string cool::lexer::Lexer::parse_multiline(
...
void cool::lexer::Lexer::trim_multiline(
...
void cool::lexer::Lexer::consume_line(
    std::string& line, unsigned line_number) noexcept {
...
std::tuple<bool, bool, bool, std::string, size_t>
cool::lexer::Lexer::extract_cool_string(
    std::string_view cool_string_appendix) const noexcept {
    ...
    if (cool_string_appendix.back() == '\\' &&
        ...
        return {false, true, null_character_found, std::move(extracted_string),
            cool_string_appendix.size()};
    } else if (cool_string_appendix.back() == '"' &&
               cool_string_appendix.size() - 1 != last_escaped_char_index) {
        return {true, true, null_character_found, std::move(extracted_string),
            cool_string_appendix.size()};
    }
...
bool cool::lexer::Lexer::extract_multiline_comment(
    std::string& cool_string_appendix,
...
cool::lexer::Lexer::find_multiline_comment_end(
...
void cool::lexer::Lexer::add_eof_error(
...
void cool::lexer::Lexer::parse_buffer(
...
bool cool::lexer::Lexer::check_default_patterns(std::cmatch& token_match,
    std::string_view& buffer, unsigned line_number) noexcept {
...
bool cool::lexer::Lexer::check_errors(
    std::string_view& buffer, unsigned line_number) noexcept {
    ...
    for (auto& [error_regex, error_msg] : error_regexes) {
        if (std::regex_match(
            ...
            buffer.remove_prefix(static_cast<size_t>(error_match[1].length()));
            return true;
        }
    }
    if (buffer[0] == '\0') {
