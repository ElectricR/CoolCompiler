...
public:
    void register_vars(std::unordered_map<std::string_view, unsigned>&& vars) noexcept {
    ...
    [[nodiscard]] unsigned get_variable(std::string_view var_name) const noexcept {
        unsigned global_offset = 0;
        for (auto it = stack.crbegin(); it != stack.crend(); ++it) {
            if (std::holds_alternative<unsigned>(*it)) {
                global_offset += std::get<unsigned>(*it);
            } else {
                auto& entry = std::get<std::unordered_map<std::string_view, unsigned>>(*it);
...
private:
    std::vector<std::variant<unsigned, std::unordered_map<std::string_view, unsigned>>> stack;
};
