...
private:
    ...
    parse_expression_lvl_1() noexcept {
        ...
        while (true) {
            ...
            if (dot_token || at_token) {
                expression = parse_dot(
                    expression.value(), at_token.has_value(), dot_token);
                if (!expression) {
                    return {};
                }
                continue;
    ...
    [[nodiscard]] std::optional<std::shared_ptr<AST::Expression>> parse_dot(
        ...
        if (with_type) {
            ...
            auto type_token_temp =
                extract_token(lexer::TokenType::TypeIdentifier);
    ...
    [[nodiscard]] std::optional<AST::NewExpression> parse_new() noexcept {
        ...
        return {
            {std::string{type_id.value().lexeme}, type_id.value().line_number}};
    }
