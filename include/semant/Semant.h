...
private:
    ...
    [[nodiscard]] std::optional<std::string> get_object_id_type(
        const auto& object_id, const auto& vars_stack) noexcept;

    [[nodiscard]] auto get_class_fields(
    ...
    void check_class_features(
        auto& class_, auto& vars_stack, const auto& all_types) noexcept;

    void check_class_field(auto& field_feature, auto& vars_stack,
        const auto& all_types, const auto& class_) noexcept;

    void check_class_method(auto& method_feature, auto& vars_stack,
        const auto& all_types, const auto& class_) noexcept;
    ...
    [[nodiscard]] bool check_expression_callable(
    ...
    [[nodiscard]] bool check_type_permissibility(std::string curr_type,
        const auto& result_type, const auto& all_types,
        const auto& class_) noexcept;
