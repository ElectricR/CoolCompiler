...
class LexerTest(unittest.TestCase):
    def check(self, *file_name):
        ...
        reference_parser_proc = subprocess.Popen(["./reference/bin/parser"], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        reference_out = reference_parser_proc.communicate(reference_lexer_out)[0]

        my_proc = subprocess.Popen(["./parser", *file_name], stdout=subprocess.PIPE)
        my_out = my_proc.communicate()[0]
        self.assertEqual(reference_out.decode("UTF-8"), my_out.decode("UTF-8"))
        self.assertEqual(reference_parser_proc.returncode, my_proc.returncode)
